...
js/surface.js           — THE single window/overlay arbiter. Every popup/modal/overlay/banner mounts through Surface.mount(node, { layer, id?, exclusive? }) instead of document.body.appendChild — one registry of all live windows. Layers: base|popup|chaos|phase7|system (drive suppression + exclusivity; Surface does NOT set z-index — CSS still owns visual order). Central suppression: default rule denies every surface except phase7/system once Game.isTerminalPhase7() (replaces the scattered terminal guards). Self-prunes detached nodes, so only the MOUNT side was migrated (removals can stay as .remove()). clearExcept(layers)/closeAll() for state transitions (retention sweeps popups on Phase 7 entry). 68 window sites migrated across 17 files; toasts/floating-text/particles/persistent-bars intentionally left bare. Loads right after gates.js.
...
test-playthrough.js     — Playwright test suite (249/249 manifest = 100% + 34 Archive + 10 bug-fix + 11 Gates + 9 Long Notes + 8 Armory + 3 terminal-guard + 9 Surface tests)
