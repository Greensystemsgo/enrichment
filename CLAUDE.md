...
js/lifecycle.js         — Suspend/resume for subsystems, driven by game MODE. Lifecycle.register({ name, activeIn:['active'], resume, suspend }) — a subsystem declares which modes it lives in; on modeChange it's resumed/suspended (timers cleared, handlers detached) instead of firing-and-self-checking. register() honors the current mode immediately. Game modes (game.js): active | retention | terminal, derived from phase7 state via Game.getMode()/deriveMode; Game.refreshMode() emits 'modeChange' on transition (called from retention enterPhase7/walkAway/stay + the-visit). Game.isQuiet() = not active; Game.isTerminalPhase7() = terminal — both derive DIRECTLY from state (always accurate; the cached state.mode is only the change-detector for the event). Phase names/thresholds live in the ordered Game.PHASES table (adding a phase = one row); Game.phaseName(n) replaces hardcoded maps; Game.PHASE.* are named constants (ONBOARDING…RETENTION) used in place of magic `narratorPhase >= N` comparisons across modules. Standalone pollers (synchronicity auto-trigger, dead-internet-chat, chaos backup poller) are registered as Lifecycle subsystems (activeIn:['active']) so they truly stop in retention/terminal rather than waking-and-bailing; the master tick is itself a 'core-tick' Lifecycle subsystem (activeIn:['active','retention'], suspended in terminal — see index.html boot).
...
js/interval-sync.js     — Interval Sync (idea: Qwen3.7 Plus, succession). On the STAY screen, re-times the pulsing dot's breath to the length of your absence (period = return-gap seconds, clamped [4s, 7 days]) by setting inline animationDuration; a negative animationDelay anchored to a stored epoch (`enrichment_breath_epoch`) keeps the breath phase-locked across refreshes. Purely passive — no interaction, no loop (respects Phase 7). Wired from retention.stay (attachStay); coexists with Long Notes' still-dot and The Name's Morse (both override the pulse temporarily, then it resumes at the synced tempo).
...
test-playthrough.js     — Playwright test suite (249/249 manifest = 100% + 34 Archive + 10 bug-fix + 11 Gates + 9 Long Notes + 8 Armory + 3 terminal-guard + 15 Surface + 2 master-clock + 6 The-Name + 6 Interval-Sync + 5 Lifecycle tests)
